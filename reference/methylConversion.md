...
These functions convert methylation M-values to beta values and vice
versa. Beta values represent methylation levels as proportions (0-1),
while M-values are log2 ratios that provide better statistical
...
For `convert_m_to_beta`, returns a matrix of beta values corresponding
to the input M-values.

For `convert_beta_to_m`, returns a matrix of M-values corresponding to
the input beta values.
...
**Numerical Stability:** The functions automatically handle extreme beta
